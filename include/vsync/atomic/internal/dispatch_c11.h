...
#define V_DISPATCH_C11_APPLY(ptr_t, val_t, func, ...)                         \
    V_NR_VARS(V_DISPATCH_C11_APPLY_, __VA_ARGS__)(ptr_t, val_t, func,         \
                                                  __VA_ARGS__)
...
#define V_DISPATCH_C11_APPLY_PTR(ptr_t, func, ...)                            \
    V_NR_VARS(V_DISPATCH_C11_APPLY_PTR_, __VA_ARGS__)(ptr_t, func,            \
                                                      __VA_ARGS__)
...
#define V_DISPATCH_C11_CASE_PTR(P, F, mo, a, ...)                             \
    V_DISPATCH_C11_APPLY_PTR(V_MAP_A(P) *, V_FUNC(P, F, mo), a,               \
                             ##__VA_ARGS__)
...
#define V_DISPATCH_C11_IS_SUPPORTED_CONST(a)                                  \
    _Generic((a),                                                             \
        vatomic64_t *: 1, const vatomic64_t *: 1,                             \
        vatomic32_t *: 1, const vatomic32_t *: 1,                             \
        vatomic16_t *: 1, const vatomic16_t *: 1,                             \
        vatomic8_t *: 1, const vatomic8_t *: 1,                               \
        vatomicptr_t *: 1, const vatomicptr_t *: 1,                           \
        vatomicsz_t *: 1, const vatomicsz_t *: 1,                             \
        default: 0)

#define V_DISPATCH_C11_UNSUPPORTED_CHECK(cond)                                 \
    (V_DISPATCH_EXPR_ASSERT(cond), 0)

#define V_DISPATCH_C11_ALL_CASES(F, mo, a, ...)                               \
    vatomic64_t *: V_DISPATCH_C11_CASE(vatomic64, F, mo, a, ##__VA_ARGS__),   \
    vatomic32_t *: V_DISPATCH_C11_CASE(vatomic32, F, mo, a, ##__VA_ARGS__),   \
    vatomic16_t *: V_DISPATCH_C11_CASE(vatomic16, F, mo, a, ##__VA_ARGS__),   \
    vatomic8_t *: V_DISPATCH_C11_CASE(vatomic8, F, mo, a, ##__VA_ARGS__),     \
    vatomicptr_t *:                                                           \
        V_DISPATCH_C11_CASE_PTR(VSYNC_DISPATCH_PTR_PREFIX, F, mo, a,          \
                                ##__VA_ARGS__),                               \
    vatomicsz_t *:                                                            \
        V_DISPATCH_C11_CASE(VSYNC_DISPATCH_SZ_PREFIX, F, mo, a,               \
                            ##__VA_ARGS__)
...
#define V_DISPATCH_C11_ALL_CONST_CASES(F, mo, a, ...)                         \
    ...
    const vatomic32_t *:                                                      \
        V_DISPATCH_C11_CASE_CONST(vatomic32, F, mo, a, ##__VA_ARGS__),        \
    const vatomic16_t *:                                                      \
        V_DISPATCH_C11_CASE_CONST(vatomic16, F, mo, a, ##__VA_ARGS__),        \
    const vatomic8_t *:                                                       \
        V_DISPATCH_C11_CASE_CONST(vatomic8, F, mo, a, ##__VA_ARGS__),         \
    const vatomicptr_t *:                                                     \
        V_DISPATCH_C11_CASE_PTR_CONST(VSYNC_DISPATCH_PTR_PREFIX, F, mo, a,    \
                                      ##__VA_ARGS__),                         \
    const vatomicsz_t *:                                                      \
        V_DISPATCH_C11_CASE_CONST(VSYNC_DISPATCH_SZ_PREFIX, F, mo, a,         \
                                  ##__VA_ARGS__)
...
#define V_DISPATCH_C11_GENERIC_ALL_MUT(F, mo, a, ...)                         \
    _Generic((a),                                                             \
        V_DISPATCH_C11_ALL_CASES(F, mo, a, ##__VA_ARGS__),                    \
        default: V_DISPATCH_C11_UNSUPPORTED_CHECK(                            \
            V_DISPATCH_C11_IS_SUPPORTED_MUT(a)))
#define V_DISPATCH_C11_GENERIC_ALL_CONST(F, mo, a, ...)                       \
    _Generic((a),                                                             \
        V_DISPATCH_C11_ALL_CONST_CASES(F, mo, a, ##__VA_ARGS__),              \
        default: V_DISPATCH_C11_UNSUPPORTED_CHECK(                            \
...
#define V_DISPATCH_C11_GENERIC_32_CONST(F, mo, a, ...)                        \
    _Generic((a),                                                             \
        V_DISPATCH_C11_32_CONST_CASES(F, mo, a, ##__VA_ARGS__),               \
        default: V_DISPATCH_C11_UNSUPPORTED_CHECK(                            \
            V_DISPATCH_C11_IS_SUPPORTED_CONST(a)))
...
#define V_DISPATCH_ALL_RET(F, mo, a, ...)                                     \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_ALL_MUT(F, mo, a,            \
                                                         ##__VA_ARGS__))
#define V_DISPATCH_CONST_ALL(F, mo, a, ...)                                   \
    V_DISPATCH_C11_GENERIC_ALL_CONST(F, mo, a, ##__VA_ARGS__)
#define V_DISPATCH_CONST_ALL_RET(F, mo, a, ...)                               \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_ALL_CONST(F, mo, a,          \
                                                           ##__VA_ARGS__))
...
#define V_DISPATCH_INT_RET(F, mo, a, ...)                                     \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_INT_MUT(F, mo, a,            \
                                                         ##__VA_ARGS__))
...
#define V_DISPATCH_CONST_INT_RET(F, mo, a, ...)                               \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_INT_CONST(F, mo, a,          \
                                                           ##__VA_ARGS__))
...
#define V_DISPATCH32_ALL_RET(F, mo, a, ...)                                   \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_32_MUT(F, mo, a,             \
                                                        ##__VA_ARGS__))
...
#define V_DISPATCH32_CONST_ALL_RET(F, mo, a, ...)                             \
    V_DISPATCH_C11_RESULT(a,                                                  \
                          V_DISPATCH_C11_GENERIC_32_CONST(F, mo, a,           \
                                                          ##__VA_ARGS__))
